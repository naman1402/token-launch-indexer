 ...
 * @dev Simulates multi-level funding transactions to create funding history that can be indexed
 */
contract FundingSimulator is Script {
    ...
    // Addresses derived from the above keys (for logging purposes)
    address public level1Funder;
    address public level2Funder;
    address public level3Funder;
    
    /**
     * @notice Creates a 3-level funding history for the given target address
     * @param target The address receiving the funds (typically the token deployer)
     */
